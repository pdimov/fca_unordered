...
template<class K, class V> using unordered_hybrid_map_fnv1a =
  unordered_hybrid_map<
    K, V, fnv1a_hash, std::equal_to<K>,
    ::allocator<unordered_hybrid_impl::map_value_adaptor<K, V>>>;
...
int main()
{
    ...
    {
        std::cout << std::setw( 39 ) << ( x.label_ + ": " ) << std::setw( 5 ) << x.time_ << " ms, " << std::setw( 9 ) << x.bytes_ << " bytes in " << x.count_ << " allocations\n";
    }
}
